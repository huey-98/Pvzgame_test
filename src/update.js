...
(function (Game) {
  ...
  Game.spawnWaveEnemy = function () {
    ...
    if (C.enemies[type].codexCategory === "elite") {
      var elite = S.enemies[S.enemies.length - 1];
      session.message = "精英来袭：" + C.enemies[type].name + "！";
      session.messageTimer = 2.5;
      Game.burst(elite.x, elite.y, 18, C.enemies[type].accent);
    }
    session.spawnCount++;
